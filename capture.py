...
frommultisrcms = True #config.getboolean('basic','frommultisrcms')
...
dosplitcal=config.getboolean('basic','dosplitcal')
dosplittar= config.getboolean('basic','dosplittar')                               
...
# fix targets
	...
	mybpcals = myampcals
	logging.info('Flux caibrators are %s', str(myampcals))
...
#	print(ampcalscans)
	logging.info("Flux calibrator scans are:")
...
if doinitcal == True:
	...
	for i in range(0,len(myampcals)):
		default(setjy)
		setjy(vis=msfilename, spw=flagspw, field=myampcals[i], standard=std)
# Delay calibration  using the flux calibrator in the list which is less flagged
	s = flagdata(vis=msfilename, mode='summary')
	minf=1000 #var for storing min flagg per.
	mini=0 #var for cal with minimum flagg. 
	for y in myampcals:
		flagged_percent = 100.*(s['field'][y]['flagged']/s['field'][y]['total'])
		if flagged_percent<minf:
			minf=flagged_percent
			mini=y
	logging.info("Using ", mini, " for delay calib %0.2f" % minf, "% flagged.")
	if os.path.isdir(str(msfilename)+'.K1'+mycalsuffix) == True:
		os.system('rm -rf '+str(msfilename)+'.K1'+mycalsuffix)
	gaincal(vis=msfilename, caltable=str(msfilename)+'.K1'+mycalsuffix, spw =flagspw, field=mini, 
...
# Delay calibration  using the flux calibrator which is less flagged
...
# SPLIT step
#############################################################
if dosplittar == True or dosplitcal== True:
#	assert os.path.isdir(msfilename), "dosplit = True but ms file not found."
        try:
                ...
                sys.exit()
	logging.info("The data will be split into separate files.")
	casalog.filter('INFO')
# fix targets
	myfields = getfields(msfilename)
	print(myfields)
	...
	for i in range(0,len(myfields)):
		if myfields[i] in stdcals:
			...
			mypcals.append(myfields[i])
		elif dosplitcal== True:
			logging.info("You have chosen to split Flux and Phase calibrators.")
			for j in range(0,len(myampcals)):
				mytargets.append(myampcals[j])
			for k in range(0,len(mypcals)):
				mytargets.append(mypcals[k])
			if dosplittar==True:
			   logging.info("You have chosen to split Target sources as well.")
			   mytargets.append(myfields[i])
			logging.info(mytargets)			
		else:
			logging.info("You have chosen to split only the Target sources.")
			mytargets.append(myfields[i])
	...
	for i in range(0,len(mytargets)):
		if os.path.isdir(mytargets[i]+'split.ms') == True:
			...
			os.system('rm -rf '+mytargets[i]+'split.ms')
                logging.info("Splitting source data.")
